...
dur = 1 
sixtyBPMBeat = squareWave(1, dur, 0.25)
beat = importWav("out.wav")
sixtyBPMBass = combineAudio([beat[0:len(sixtyBPMBeat)], sixtyBPMBeat], multiply=True) 
delayedsixtyBPMBass = delayAudio(sixtyBPMBass, 0.3) 
sdBass = scaleVolume(delayedsixtyBPMBass, 1)
final = combineAudio([sixtyBPMBass, sdBass], add=True)
...
low = 4
high = 70
newfinal = butter_bandpass_filter(final, low, high, sd.default.samplerate, order=2)
newfinal = butter_bandpass_filter(newfinal, low, high, sd.default.samplerate, order=2)
newfinal = butter_bandpass_filter(newfinal, low, high, sd.default.samplerate, order=2)
newfinal = butter_bandpass_filter(newfinal, low, high, sd.default.samplerate, order=2)
setVolume(newfinal, 3)
"""
print(final[0:15],newfinal[0:15])
plt.subplot(212)
plt.plot(newfinal)
plt.savefig("picture.png")
print("plotting....")
os.system("cp picture.png ./../../../Downloads/")
"""
sd.play(scaleVolume(newfinal, 1), blocking=True, loop=True)

"""
waveOne = scaleVolume(sineWave(120, 4), 0.2)
waveTwo = scaleVolume(sineWave(0.5, 4), 1)
waveFour = scaleVolume(squareWave(0.5, 4, 0.5), 1)
waveTwo = list(map(lambda x:(x**2)*np.sign(x), waveTwo))
waveThree = combineAudio([waveOne, waveTwo], multiply=True)
waveFive = combineAudio([waveOne, waveFour], multiply=True)
sd.play(np.array(waveOne), blocking=True)
sd.play(np.array(waveThree), blocking=True)
sd.play(np.array(waveFive), blocking=True)
"""
